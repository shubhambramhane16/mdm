        ...
        <div style="max-width: 800px; margin: auto; background: #fff; padding: 20px; border: 1px solid #ccc;">
            <table style="width: 100%; border-bottom: 1px solid #ccc;">
                <tr>
                    ...
                    <td>
                        <h2 style="text-align: start;">Purchase Order</h2>
                ...
                <tr>
                    <td style="width: 60%; vertical-align: top;">
                        <p style="line-height: 25px; font-size: 12px;"><strong>Bill to :</strong><br>
                            {{ isset($details->vendor->name) ? $details->vendor->name : 'vendor Name' }}<br>
                            {{ isset($details->vendor->address) ? $details->vendor->address : 'vendor Address' }}<br>

                            GSTIN: {{ isset($details->vendor->gst) ? $details->vendor->gst : 'vendor GST' }}<br>

                            Mobile No. :
                            {{ isset($details->vendor->contact_no) ? $details->vendor->contact_no : 'vendor Mobile' }}<br>
                            Email ID :
                            {{ isset($details->vendor->email) ? $details->vendor->email : 'vendor Email' }}
                        </p>
                    </td>
                    <td style="vertical-align: top;">
                        <p style="line-height: 25px;font-size: 12px;color: #807b7b;">PO Details :<br>
                            Purchase No. : {{ $details->po_number ?? 'INV-001' }}<br>
                            PO Date :
                            {{ !empty($details->po_date) ? \Carbon\Carbon::parse($details->po_date)->format('d M Y') : \Carbon\Carbon::now()->format('d M Y') }}<br>

                            Valid Till:
                            {{ !empty($details->expiry_date) ? \Carbon\Carbon::parse($details->expiry_date)->format('d M Y') : \Carbon\Carbon::now()->format('d M Y') }}
                        </p>
                    </td>
                </tr>
            </table>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 12px;">
                ...
                @php
                    $items = json_decode($details->product_info, true) ?? [];
                    $subtotal = 0;
                @endphp
                @forelse($items as $key => $item)
                    ...
                    <tr @if ($key % 2 == 1) style="background-color: #f2f2f2;" @endif>
                        <td style="padding: 10px;">{{ $item['product_code'] ?? '-' }}</td>
            ...
            <table style="width: 100%; margin-top: 30px; font-size: 12px;">
                <tr>
                    <td style="width: 60%; vertical-align: top;">
                        <!-- Bank details section removed as requested -->
                    </td>
                    <td style="vertical-align: top;">
                        <table class="modern-summary-table" style="width: 100%; font-size: 12px;">
                            @php
                                $gst_rate = $details->gst ?? 18;
                                $cgst_rate = $details->cgst ?? $gst_rate / 2;
                                $sgst_rate = $details->sgst ?? $gst_rate / 2;
                                $igst_rate = $details->igst ?? 0;
                        ...
                        <table style="width: 100%; font-size: 12px;">
                            <tr class="total-row">
                                <td style="text-align: right; font-weight: bold; border: none;">GRAND TOTAL</td>
                                <td class="text-right" style="text-align: right; font-weight: bold; border: none;">
