...
<head>
    ...
    <title>Invoice #{{ $invoice->invoice_number ?? 'INV-001' }}</title>
    <style>
        body {
...
<body>
    <div class="modern-invoice-container">
        ...
        <div class="modern-header">
            <div>
                <div class="modern-company">{{ config('app.name', 'Your Company Name') }}</div>
                <div style="font-size:15px; color:#666; margin-top:6px;">
                    ...
                    GST: {{ $company_gst ?? '22AAAAA0000A1Z5' }}
                </div>
            </div>
            <div class="modern-invoice-title">
                <h2>INVOICE</h2>
                <div class="modern-status {{ $invoice->status ?? false ? 'paid' : 'pending' }}">
                    {{ $invoice->status ?? false ? 'PAID' : 'PENDING' }}
                </div>
            </div>
        </div>
        <!-- Meta -->
        <div class="modern-meta">
            <div class="modern-block">
                ...
                <div class="row"><span class="label">Invoice #</span><span
                        class="value">{{ $invoice->invoice_number ?? 'INV-001' }}</span></div>
                <div class="row"><span class="label">Date</span><span
                        class="value">{{ !empty($invoice->invoice_date) ? \Carbon\Carbon::parse($invoice->invoice_date)->format('d M Y') : \Carbon\Carbon::now()->format('d M Y') }}</span>
                </div>
                <div class="row"><span class="label">Due Date</span><span
                        class="value">{{ !empty($invoice->due_date) ? \Carbon\Carbon::parse($invoice->due_date)->format('d M Y') : \Carbon\Carbon::now()->addDays(30)->format('d M Y') }}</span>
                </div>
            ...
            <div class="modern-block">
                ...
                <div class="row"><span class="label">Company</span><span
                        class="value">{{ $client->name ?? 'Client Name' }}</span></div>
                <div class="row"><span class="label">Contact</span><span
                        class="value">{{ $client->contact_person_name ?? 'Contact Person' }}</span></div>
                <div class="row"><span class="label">Email</span><span
                        class="value">{{ $client->contact_person_email ?? 'email@client.com' }}</span></div>
                <div class="row"><span class="label">Phone</span><span
                        class="value">{{ $client->contact_person_mobile ?? '+1 (555) 987-6543' }}</span></div>
                <div class="row"><span class="label">Address</span><span
                        class="value">{{ $client->address ?? 'Client Address' }}</span></div>
                @if (!empty($client->gst))
                    <div class="row"><span class="label">GST</span><span class="value">{{ $client->gst }}</span>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="modern-summary">
            <table class="modern-summary-table">
                ...
                @endphp
                <tr>
                    <td class="label">GST ({{ $gst_rate }}%)</td>
    ...
    <script>
        function downloadPDF() {
            ...
            html2canvas(invoiceContent, {
                ...
                link.download = 'Invoice-{{ $invoice->invoice_number ?? 'INV-001' }}.png';
                link.click();
